...
sparsity = 20  # NELL-955 only has 20% sparsity data
...
def load_conrad_data(query_type):
    """Load Conrad recall and latency data from CSV and log files."""
    dir_name = f"conrad_bench_nell-955_{query_type}_{sparsity}"
...
def load_neural_data(query_type):
    """Load Neural baseline recall and latency data from single CSV."""
    dir_name = f"neural_bench_nell-955_{query_type}_{sparsity}"
...
def load_symbolic_data(query_type):
    """Load Symbolic baseline recall and latency data (single point)."""
    dir_name = f"symbolic_bench_nell-955_{query_type}_{sparsity}"
    csv_path = base_path / dir_name / "baseline_results_summary.csv"
...
def load_hybrid_data(query_type):
    """Load Hybrid baseline recall and latency data (multiple threshold points)."""
    dir_name = f"hybrid_bench_nell-955_{query_type}_{sparsity}"
    csv_path = base_path / dir_name / "baseline_results_summary.csv"
...
for q_type in query_types:
    ...
    # Load Conrad data
    c_r, c_l = load_conrad_data(query_pipeline)
    print(f"Loaded conrad {q_type}: {len(c_r) if c_r else 0} points")
    
    # Load Neural data
    n_r, n_l = load_neural_data(query_pipeline)
    print(f"Loaded neural {q_type}: {len(n_r) if n_r else 0} points")
    
    # Load Symbolic data
    s_r, s_l = load_symbolic_data(query_pipeline)
    print(f"Loaded symbolic {q_type}: {'yes' if s_r is not None else 'no'}")
    
    # Load Hybrid data
    h_r, h_l = load_hybrid_data(query_pipeline)
    print(f"Loaded hybrid {q_type}: {len(h_r) if h_r else 0} points")
...
for col, q_type in enumerate(query_types):
    ax = axes[col]
    
    # Plot Symbolic
    s_data = data[q_type]["symbolic"]
    ...
    if h_data[0] is not None and h_data[1] is not None:
        ...
        for i, (r, l) in enumerate(zip(h_r, h_l)):
            ...
            label = f'hybrid (t={hybrid_thresholds[marker_idx]})' if col == 0 and i < len(hybrid_thresholds) else None
    ...
    ax.set_xlim(0.1, 1)
    ax.set_title(f"{q_type.upper()}", fontsize=12)
...
# plt.suptitle("NELL-955 (20% Missing)", fontsize=14, y=1.02)
plt.subplots_adjust(wspace=0.25)
# Create figure-level legend positioned at the top center
fig.legend(handles, labels, loc='upper center', ncol=5, fontsize=8.5, bbox_to_anchor=(0.4, 1.45))
plt.savefig('baselines_recall_vs_latency_nell955_from_csv.png', dpi=300, bbox_inches='tight')
print("Plot saved to baselines_recall_vs_latency_nell955_from_csv.png")
