...
# Configuration for the 1x8 plot (NELL-955 at 20% sparsity)
# Single row: 20% sparsity
# Columns: 3 original templates (3p, 2u, 2ip) + 5 new templates (2p, 2i, pi, 3i, up)
datasets = [("NELL-955", "nell-955")]
...
NEW_BASE = Path("/data/sonia/conrad/artifacts/benchmark")

templates = [
    # Original 3
    ("ThreeHop",           "3p", '#1f77b4', 'o', ORIGINAL_BASE),
    ("TwoUnion",           "2u", '#2ca02c', 's', ORIGINAL_BASE),
    ("TwoIntersectProject","2ip",'#ff7f0e', '^', ORIGINAL_BASE),
    # New 5
    ("TwoHop",             "2p", '#9467bd', 'o', NEW_BASE),
    ("TwoIntersect",       "2i", '#d62728', 's', NEW_BASE),
    ("ProjectIntersect",   "pi", '#8c564b', '^', NEW_BASE),
    ("ThreeIntersect",     "3i", '#e377c2', 'D', NEW_BASE),
    ("UnionProject",       "up", '#17becf', 'v', NEW_BASE),
]

sparsity_values = [20]
sparsity_labels = ["20% sparsity"]
...
# --- Plotting Setup ---
# 1x8 grid: single row (20% sparsity), columns = query templates
n_cols = len(templates)
fig, axes = plt.subplots(1, n_cols, figsize=(13, 2.2), sharex=True, sharey=True)
...
for r, sparsity_val in enumerate(sparsity_values):  # Single row: 20% sparsity
    ...
    for c, (query_type, query_label, query_color, query_marker, base_path) in enumerate(templates):
        ...
        # X-axis labels (single row, always bottom)
...
for query_type, query_label, query_color, query_marker, _ in templates:
    handles.append(Line2D([0], [0], color=query_color, marker=query_marker, linestyle='-',
                         linewidth=2.5, markersize=5, label=query_label))
...
plt.savefig('conrad_validity_nell_20pct_with_ci.pdf', dpi=300, bbox_inches='tight')
plt.savefig('conrad_validity_nell_20pct_with_ci.png', dpi=300, bbox_inches='tight')

print("Plot saved to conrad_validity_nell_20pct_with_ci.png")
