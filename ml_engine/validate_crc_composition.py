...
class BenchmarkConfig:
    ...
    def __post_init__(self):
        ...
        # Get repo root (assuming script is in ml_engine/, go up one level)
        if self.dataset:
            ...
            # Get repo root: go up from ml_engine/ to repo root
            script_dir = os.path.dirname(os.path.abspath(__file__))
            repo_root = os.path.dirname(script_dir)
            
            # Construct dataset-specific paths
            if self.calibration_data_base is None:
                self.calibration_data_base = os.path.join(repo_root, "queries", dataset_normalized, "calibration_data")
            if self.test_data_base is None:
                self.test_data_base = os.path.join(repo_root, "queries", dataset_normalized, "test")
        else:
...
class CRCBenchmarkValidator:
    ...
    def setup_model_and_calibration(self, inf_args) -> Tuple[ModelFactory, Any]:
        ...
        logging.info("Setting up model and performing calibration")
...
def main():
    ...
    if args_validate_crc.dataset:
        ...
        if args_validate_crc.dataset in dataset_to_model_path:
            # Get the root directory (ml_engine -> .. -> orb root)
