...
from utils import get_embeddings, get_graph


class InferenceFactory:
    def __init__(self, inference_args, vectordb_controller, model_specific_args):
        ...
        if self.model_to_infer.lower() == "query2box":
            self.shape_input_fn = query2box_shape_input
        elif self.model_to_infer.lower() == "transr":
            self.shape_input_fn = transr_shape_input
        elif self.model_to_infer.lower() == "ultra":
            self.shape_input_fn = ultra_shape_input
        elif self.model_to_infer.lower() == "relationprediction":
            self.shape_input_fn = relationprediction_shape_input
        ...
        self.inited_inputs = init_inputs(
            db_controller, self.vectordb_controller, self.device, self.input_fn_args
        )
    ...
    def shape_input(self, inputs):
        ...
        - 'query2box': Uses the `shape_query2box_input` method to shape the input.
        - 'transr': Uses the `shape_transr_input` method to shape the input.
        """
...
def init_inputs(db_controller, vectordb_controller, device, input_fn_args):
    inputs = {}
    if "db_controller" in input_fn_args:
        inputs["db_controller"] = db_controller
    if "vectordb_controller" in input_fn_args:
        inputs["vectordb_controller"] = vectordb_controller
    ...
    if "entity_embedding" in input_fn_args:
        ...
        inputs["entity_embedding"] = get_embeddings(
            vectordb_controller, all_node_ids, device
        )
