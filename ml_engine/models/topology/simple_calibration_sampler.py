...
python simple_calibration_sampler.py --num-queries 500 --size-ratio 0.05

# Generate 2000 queries per hop with 20% size ratio  
python simple_calibration_sampler.py --num-queries 2000 --size-ratio 0.2

# Generate 2-hop and 3-hop queries
python simple_calibration_sampler.py --min-hops 2 --max-hops 3

# Filter queries with intermediate hops and final answers having more than 30 nodes
python simple_calibration_sampler.py --max-hop-size 30

# Verbose output for debugging
python simple_calibration_sampler.py --verbose

# Generate 3-hop queries AND 2ip queries
python simple_calibration_sampler.py --generate-2ip --num-2ip-queries 1000

# Generate only 2ip queries (use simple_2ip_sampler.py directly)
python simple_2ip_sampler.py --num-queries 1000 --max-answers 1000
"""
...
# Use absolute import since all files are in the same directory
_current_dir = os.path.dirname(os.path.abspath(__file__))
if _current_dir not in sys.path:
    sys.path.insert(0, _current_dir)
from simple_3p_sampler import generate_3p_queries, build_ent_out_structure
from simple_2ip_sampler import generate_2ip_queries
from simple_2u_sampler import generate_2u_queries
import random
