...
class DBExecModel(nn.Module, ModelUtils):
    ...
    def __init__(self, args: Namespace, num_relations: int, device: str):
        ...
        # path = Path("/home/sfhor/orb-dev/orb/benchmark/edge_type_stats.json")
        # with path.open("r", encoding="utf-8") as f:
        #     self.edge_popularity = json.load(f)

        # path = Path("/home/sfhor/orb-dev/orb/benchmark/node_degree_stats.json")
        # with path.open("r", encoding="utf-8") as f:
        #     self.node_degree_popularity = json.load(f)
    ...
    def _compute_scores(self, results, rel_type):
        ...
        # edge_popularity = self.edge_popularity.get(str(rel_type), {}).get("popularity")
    ...
    def generateCalibrateSamples(
        ...
        for i in tqdm(range(0, len(queries), batch_size), desc="Processing batches"):
            ...
            batch_results, _ = self.predict(batch_queries)
            
            # Process each query result in the batch
            for query, results in zip(batch_queries, batch_results):
                prob = torch.zeros(14541)
                
                # Compute scores using the same logic as other methods
                query_scores = self._compute_scores(results, query[1].item())
                for node_id, score in query_scores.items():
                    prob[node_id] = score
