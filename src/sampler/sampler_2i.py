#!/usr/bin/env python3
"""
Script to generate 2-way intersection (2i) queries.

Query pattern: 2 projections from different anchors → intersection
Example: "Which entities are reachable from X via R1 AND from Y via R2?"
- Anchor1 → rel1 → set1
- Anchor2 → rel2 → set2
- Answers = set1 ∩ set2

Query format: ((anchor1, rel1), (anchor2, rel2), "2i")
"""
...
def generate_2i_queries(
    graph_data,
    num_queries=1000,
    max_answers=1000,
    min_answers=1,
    max_hop_size=None,
    save_path="./calibration_data/2i_pipeline",
):
    """Generate 2i queries from the graph."""
    ...
    # Build reverse index: entity → [(anchor, rel), ...] to find candidate intersections
    ...
    intersection_candidates = {
        entity: pairs for entity, pairs in entity_to_anchors.items() if len(pairs) >= 2
    }

    if not intersection_candidates:
        raise ValueError("No entities found that can serve as intersections. Graph may be too sparse.")

    logging.info(f"Found {len(intersection_candidates)} potential intersection entities")
    ...
    sample_progress = tqdm.tqdm(total=num_queries, desc=f"Generating 2i queries")
    ...
    while num_sampled < num_queries and num_try < max_attempts and consecutive_failures < max_consecutive_failures:
        ...
        sample_progress.set_description(
            f"Generating 2i queries: {num_sampled}/{num_queries} (tried: {num_try})"
        )

        intersection_entity = random.choice(list(intersection_candidates.keys()))
        unique_pairs = list(set(intersection_candidates[intersection_entity]))
        ...
        query = ((anchor1, rel1), (anchor2, rel2), "2i")
    ...
    logging.info(f"Generated {num_sampled}/{num_queries} queries (total attempts: {num_try})")
...
def sample_2i_calibration_data(
    num_queries=1000,
    max_answers=1000,
    min_answers=1,
    ...
    logging.info("Starting 2i query generation...")
    ...
    queries, answers = generate_2i_queries(
        graph_data,
    ...
    logging.info(f"2i query generation completed! Generated {len(queries)} queries.")
    return save_path


def main():
    parser = ArgumentParser(description="Generate 2i (intersection) queries")
    ...
    parser.add_argument("--save-path", type=str, default="./calibration_data/2i_pipeline")
    ...
    try:
        save_path = sample_2i_calibration_data(
        ...
        print(f"\n2i queries saved to: {save_path}")
    except Exception as e:
        logging.error(f"Error during 2i query generation: {e}")
        raise
