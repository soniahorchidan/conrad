...
MODELS=("ThreeHopPipeline" "TwoUnionPipeline" "TwoIntersectProjectPipeline")
...
# Model-specific calibration batch sizes
declare -A CALIB_BATCH_SIZES=(
    ["ThreeHopPipeline"]=8
    ["TwoUnionPipeline"]=8
    ["TwoIntersectProjectPipeline"]=8
)
...
for incompleteness in "${INCOMPLETENESS_LEVELS[@]}"; do
    log INFO "=========================================="
    log INFO "Processing incompleteness level: ${incompleteness}%"
    log INFO "=========================================="
    ...
    log INFO "Completed all datasets for incompleteness level: ${incompleteness}%"
done
